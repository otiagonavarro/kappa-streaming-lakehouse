...
@click.option("--session-gap-minutes", default=2, type=int, help="Session gap timeout in minutes")
def main(from_beginning: bool, session_gap_minutes: int):
    ...
    # Session window aggregation into Iceberg (legacy GROUP BY SESSION syntax)
    t_env.execute_sql(f"""
        INSERT INTO kappa.session_metrics
        SELECT
            session_id,
            user_id,
    ...
    # Sink to PostgreSQL with upsert (idempotent reprocessing)
    t_env.execute_sql(f"""
